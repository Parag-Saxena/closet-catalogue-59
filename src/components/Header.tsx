...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full morphism-card border-0 border-b border-border/50 safe-area">
      <div className="container-responsive flex items-center justify-between h-16">
        <div className="flex items-center gap-3">
          ...
          >
            <div className="morphism-button p-2">
              <Shirt className="h-6 w-6 text-primary" />
            </div>
            <span className="heading-sm text-foreground hidden sm:block">Closet Keeper</span>
          </Link>
          ...
          {user && (
            <Button 
              ...
              className={cn(
                "morphism-button h-10 w-10 hidden lg:flex",
          ...
          {user && (
            <Button 
              ...
              onClick={toggleSidebar}
              className={cn(
                "morphism-button h-10 w-10 lg:hidden",
                "hover:scale-105 active:scale-95"
              )}
          ...
          <nav className="hidden xl:flex items-center space-x-1 ml-6">
            <Link 
              to="/home" 
              className={cn(
                "px-4 py-2 body-md font-medium rounded-lg transition-all duration-200",
                "hover:bg-accent/50 hover:text-primary interactive-morphism"
              )}
            >
              Home
            </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  variant="ghost" 
                  className={cn(
                    "morphism-button flex items-center gap-3 h-10 px-3",
                    "hover:scale-105 active:scale-95"
                  )}
                >
                  ...
                  <span className="body-md font-medium hidden md:inline truncate max-w-24">
                    {user.name}
                  </span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent 
                align="end" 
                className="w-56 morphism-card border-border/50 p-2"
              >
                <div className="flex items-center gap-3 p-3 rounded-lg bg-accent/20">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="body-md font-medium text-foreground truncate">{user.name}</p>
                    <p className="body-sm text-muted-foreground truncate">{user.email}</p>
                  </div>
                </div>
            ...
            location.pathname !== '/sign-in' && location.pathname !== '/sign-up' && (
              <Button asChild className="morphism-button hover:scale-105 active:scale-95">
                <Link to="/sign-in">
